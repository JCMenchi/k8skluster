...
for ((i=0;i<$nb;i++)); do
  ...
  n=$(az network route-table route list -o table -g ${RESGROUP} --route-table-name k8sroutes | grep ${ROUTE_CIDR[$i]} | wc -l)
  if [ $n = 1 ]; then
    # check if hop is the same
    n=$(az network route-table route list -o table -g ${RESGROUP} --route-table-name k8sroutes | grep ${ROUTE_CIDR[$i]} | grep ${ROUTE_NEXT_HOP[$i]} | wc -l)
    if [ $n = 1 ]; then
